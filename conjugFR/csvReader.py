...
def Preterito_imperfecto_de_indicativo():
    with open('conjugFR/times/Pretérito imperfecto de indicativo.csv', newline='', encoding='utf8',
...
def Pretero_imperfecto_de_subjonctivo():
    with open('conjugFR/times/Prétero imperfecto de subjonctivo.csv', newline='', encoding='utf8',
              errors='ignore') as csvfile:
        reader = csv.reader(csvfile, delimiter=';')
        rows = [row for row in reader]
